...
class RecordThread(threading.Thread):
    ...
    def run(self):
        ...
        # srt top center
        self.srt("{}\n00:00:00,000 --> 00:00:01,000\n<font color=#FFFF00>录制日期：{}</font>".format(str(rt.line),str(time.strftime("%Y-%m-%d",rt.localtime))))import threading
import pyaudio
import copy
import math
import time
...
class RecordThread(threading.Thread):
    ...
    def run(self):
        ...
        # srt top center
        self.srt("{}\n00:00:00,000 --> 00:00:03,000\n<font color=#FFFF00>通联日期：{}</font>".format(str(rt.line),str(time.strftime("%Y-%m-%d",rt.localtime))))
        self.srt("{\\an5}请遵守<font color=#FF0000><u><b>《中华人民共和国无线电管理条例》</b></u></font>\n")
        self.line = self.line + 1
        print("RUNNING ... Started at {}".format(rt.localtimestr))

        while self.bRecord:
            data = self.wavstream.read(self.chunk)
...
if __name__ == '__main__':
    try:
        ...
        while True:
            ...
            # if quiet over x seconds or sat @20 o'clock
            if (rt.stoptime > 0 and time.time() - rt.stoptime > 600 ) \
            or (localtime.tm_wday == 5 and localtime.tm_hour == 19 and localtime.tm_min == 55): 
                # restart record
                rt.bRecord = False
                rt = RecordThread()
                rt.start()
